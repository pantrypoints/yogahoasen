...
<a href="/articles/{article.slug}"
 ...
 <script lang="ts">
  ...
  // Show localised title/excerpt if available, fall back to English
  const displayTitle = $derived(
    $locale === 'vi' && article.titleVi ? article.titleVi : article.title
  );
  const displayExcerpt = $derived(
    $locale === 'vi' && article.excerptVi ? article.excerptVi : article.excerpt
  );
</script>
...
>
  ...
  <div class="flex flex-col flex-1 p-5">
    ...
    <h3 class="font-semibold text-neutral-900 dark:text-neutral-100 leading-snug mb-2
