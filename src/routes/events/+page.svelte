...
<script lang="ts">
  ...
  import EventCard from '$lib/components/EventCard.svelte';
  import { events } from '$lib/data/events';
  import { Search, X, CalendarDays, Calendar, Clock } from 'lucide-svelte';
  import { fly, fade } from 'svelte/transition';
    ...
    <div in:fly={{ y: 15, delay: 150, duration: 500 }} class="max-w-xl mx-auto">
      <div class="relative">
        ...
        <input
          ...
          bind:value={query}
          placeholder={$t('events.search')}
          class="w-full pl-12 pr-12 py-3.5 rounded-2xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100 focus:border-lotus-400 focus:ring-0 text-sm shadow-sm"
        />
        {#if query}
          <button
            onclick={() => query = ''}
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
  ...
  <div class="flex flex-wrap gap-2 mb-10 justify-center">
    {#each categories as cat}
      <button
        onclick={() => activeCategory = cat}
        class="px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 {activeCategory === cat
          ? 'bg-lotus-600 text-white shadow-md'
          : 'bg-stone-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300 hover:bg-lotus-50'}"
      >
        {cat}
  ...
  {:else}
    ...
    {#if upcomingEvents.length > 0}
      <div class="mb-16">
        <div class="flex items-center gap-3 mb-8">
          <div class="w-10 h-10 rounded-full bg-lotus-100 dark:bg-lotus-900/30 flex items-center justify-center">
            <Calendar class="w-5 h-5 text-lotus-600 dark:text-lotus-400" />
          </div>
          <h2 class="text-2xl font-heading font-bold text-neutral-900 dark:text-neutral-100">
            {$t('events.upcoming')}
          </h2>
          <span class="px-2.5 py-1 rounded-full text-xs font-semibold bg-lotus-100 dark:bg-lotus-900/30 text-lotus-700 dark:text-lotus-300">
    ...
    {#if pastEvents.length > 0}
      <div>
        <div class="flex items-center gap-3 mb-8">
          <div class="w-10 h-10 rounded-full bg-stone-200 dark:bg-neutral-800 flex items-center justify-center">
            <Clock class="w-5 h-5 text-stone-600 dark:text-stone-400" />
          </div>
          <h2 class="text-2xl font-heading font-bold text-neutral-900 dark:text-neutral-100">
            {$t('events.past')}
          </h2>
          <span class="px-2.5 py-1 rounded-full text-xs font-semibold bg-stone-200 dark:bg-neutral-800 text-stone-700 dark:text-stone-300">
            {pastEvents.length}
          </span>
        </div>
    ...
    <!-- Show message if no events in current filter -->
    {#if upcomingEvents.length === 0 && pastEvents.length === 0}
      <div in:fade class="text-center py-12 text-neutral-400">
        <p>{$t('events.noEventsInCategory')}</p>
      </div>
    {/if}
  {/if}
</div>